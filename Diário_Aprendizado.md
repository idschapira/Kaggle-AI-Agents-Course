...
**Status:** Codelab "Deploy ADK agent to Agent Runtime using Agents CLI" **concluído de ponta a ponta** — deploy, smoke test via SDK, teste manual no Playground, e observabilidade via Cloud Trace, todos validados em produção real. **Decisão consciente:** Seção 11 (cleanup/exclusão dos recursos) **não será executada** — o próprio codelab orienta participantes do Kaggle a manterem a infraestrutura ativa para o próximo laboratório (frontend web). BigQuery Analytics (Seção 9, item opcional) fica pendente — infraestrutura nunca aplicada, ver acima. Pendente: (1) commitar/pushar `CLAUDE.md`, `Diário_Aprendizado.md`, `expense_agent/config.py`, `expense_agent/agent_runtime_app.py`; (2) avaliar se commita também os arquivos remanescentes do `scaffold enhance` (Terraform, `deployment_metadata.json`, etc.).
