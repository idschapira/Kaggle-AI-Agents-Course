...
**Atualização (2026-06-23, continuação) — o mesmo erro voltou ao "matar e abrir um terminal novo", causa raiz diferente:**
- Mesmo com a correção de PATH já salva em `~/.bashrc`, ao fechar o terminal e abrir outro o `agents-cli: command not found` reapareceu. Diagnóstico: `cat ~/.profile | grep -A3 bashrc` confirmou que o encadeamento `~/.profile → ~/.bashrc` está correto e intacto; `source ~/.bashrc` resolveu instantaneamente (`which agents-cli` voltou a apontar pro binário certo).
- **Conclusão:** o `~/.bashrc`/`~/.profile` nunca estavam quebrados — o "terminal novo" problemático não era um shell interativo de login de verdade (ex.: ícone do Ubuntu/Windows Terminal), e sim, com grande probabilidade, a aba **"Code" embutida do Cowork**, que executa cada comando como processo isolado/não-interativo e não necessariamente carrega `.bashrc`/`.profile` da mesma forma que um terminal real — o mesmo padrão de inconsistência já visto antes (a vez em que essa aba rodou em PowerShell em vez de WSL sem avisar).
- **Lição para o Capstone:** toda checagem sensível de PATH/autenticação deve ser feita num terminal WSL "de verdade" (aberto direto, fora de qualquer agente de IA) — só depois de confirmado ali, abrir a sessão do Claude Code/aba Code, que herda o ambiente já correto. Nunca confiar que uma aba de agente embutida reflete o mesmo ambiente de shell de um terminal nativo.
- Nota cosmética: a linha de PATH ficou duplicada no `~/.bashrc` (não quebra nada, mas fica pra limpar com `awk '!seen[$0]++' ~/.bashrc > /tmp/bashrc.clean && mv /tmp/bashrc.clean ~/.bashrc` quando for conveniente).

**Seção 3 — reconfirmada com sucesso (2026-06-23), já no protocolo corrigido:** rodado o prompt oficial do codelab via Claude Code, no terminal WSL puro (sem nenhum agente envolvido na etapa de autenticação): `uvx google-agents-cli setup` → `agents-cli v0.5.0` instalado (v0.5.1 disponível), auth detectada automaticamente via variável de ambiente `GEMINI_API_KEY` (sem precisar digitar nada), 7 skills do ADK instaladas globalmente e symlinkadas no Claude Code (`adk-code`, `scaffold`, `deploy`, `eval`, `observability`, `publish`, `workflow`). Zero exposição de chave — confirma que o protocolo novo do `CLAUDE.md` funcionou de ponta a ponta.

**Atualização (2026-06-23) — Bug de ambiente split Windows/WSL no `agents-cli deploy`, resolvido, e DEPLOY REAL CONCLUÍDO COM SUCESSO:**
- Ao rodar o `agents-cli deploy --deployment-target agent_runtime` de verdade (não dry-run), voltou o mesmo erro de ADC ausente mesmo já com `gcloud auth application-default login` confirmado num terminal WSL puro. Diagnóstico do Claude Code: o binário `agents-cli` em uso era a build **Windows** (`cpython-3.11-windows-x86_64`), não a do WSL — e o `gcloud` só está instalado no WSL. Processo Windows procura ADC em `%APPDATA%\gcloud\` (não existe); o arquivo real está em `~/.config/gcloud/application_default_credentials.json`, dentro do filesystem do WSL.
- **Causa raiz:** ambiente dividido entre Windows e WSL — exatamente o tipo de inconsistência já catalogada nos bugs de PATH anteriores desta sessão (aba "Code" embutida ora roda em PowerShell, ora em WSL, sem aviso).
- **Correção aplicada (workaround, não a correção definitiva):** apontar `GOOGLE_APPLICATION_CREDENTIALS` para o caminho UNC do arquivo de credencial do WSL, visível do lado Windows via `\\wsl$\`:
  ```bash
  ...
  ```
  Importante: isso é só um **caminho de arquivo**, nunca o conteúdo da credencial — não viola a regra de segurança de nunca expor segredos no chat. Esse fix já foi adicionado pelo Claude Code na seção "Atenção — split WSL/Windows" do `CLAUDE.md`, para sessões futuras herdarem automaticamente.
- **Lição para o Capstone (mais importante que o workaround):** isso é um remendo, não a correção real. O ideal para o Capstone é **unificar o ambiente** — rodar `gcloud` e `agents-cli` no mesmo SO (os dois no WSL, ou os dois no Windows nativo), evitando depender da ponte `\\wsl$\` toda vez que houver deploy.
- **DEPLOY CONCLUÍDO COM SUCESSO:**
  - Agent Runtime ID: `projects/17805413958/locations/us-east1/reasoningEngines/1821600496155099136`
  - Service Account: `service-17805413958@gcp-sa-aiplatform-re.iam.gserviceaccount.com`
  - Console Playground: https://console.cloud.google.com/vertex-ai/agents/agent-engines/locations/us-east1/agent-engines/1821600496155099136/playground?project=kaggle-dia5-agent-runtime
- **Dica do codelab para o Capstone (registrar para referência futura):** se houver problema de deploy, considerar trocar de região. O [Region Picker do Google Cloud](https://cloud.withgoogle.com/region-picker/) ajuda a escolher a melhor região por latência, preço e pegada de carbono — útil ter à mão antes do deploy do Capstone, especialmente se a região padrão (`us-east1` neste caso) apresentar erro de quota/disponibilidade.

**Atualização (2026-06-23) — Smoke test no agente em produção: 2 bugs encontrados e corrigidos, ambos os casos PASS:**

Teste pedido: caso 1 (despesa de refeição de $50, esperando aprovação automática) e caso 2 (jantar com cliente de $150, esperando disparo do pause de HITL), chamando o Agent Runtime real via SDK (`vertexai.agent_engines`).

- **Bug 1 — região sem suporte a Gemini via Vertex AI:** o Agent Runtime roda em `us-east1` (região válida para o serviço), mas `us-east1` **não tem suporte a modelos Gemini via Vertex AI** — só Agent Runtime. O caso 2 (que precisa chamar o `review_agent`/LLM) caiu em 404 ao tentar achar o modelo. **Fix:** setar `GOOGLE_CLOUD_LOCATION=us-central1` como env var do serviço implantado, para as chamadas de modelo serem roteadas pra uma região com suporte — sem mudar a região do Agent Runtime em si. **Esta é exatamente a situação que o codelab avisou ("Region Picker") e que já tínhamos registrado preventivamente acima — confirmado na prática.**
- **Bug 2 — nome de modelo indisponível nesse contexto:** `gemini-3.1-flash-lite` foi usado com sucesso em outro codelab, então **não é nome inventado** (existe de fato, via Gemini API/AI Studio e via Vertex AI para clientes enterprise) — o erro aqui foi disponibilidade específica de região/projeto no Vertex AI no momento do teste, não o modelo não existir. **Fix:** troca por `gemini-2.5-flash-lite` em `config.py`, que está disponível nesse projeto/região. **Lição corrigida:** antes de cravar "modelo não existe", checar se é (a) nome realmente inventado/desatualizado por knowledge cutoff, ou (b) modelo real mas com disponibilidade restrita por região/plataforma (Vertex AI vs. Gemini API) — são causas diferentes com fixes diferentes.
- **Tentativa extra que não era necessária:** o Claude Code também tentou passar `vertexai.init(location=...)` explicitamine `agent_runtime_app.py` antes de fixar o Bug 1 só com a env var. Essa mudança causou falha no update do deploy e foi revertida — a env var `GOOGLE_CLOUD_LOCATION` já era suficiente, sem precisar tocar no código do `set_up()`. **Lição:** ao depurar, preferir a correção mais simples (env var) antes de editar código; só editar código se a env var sozinha não resolver.
- **Resultado final (ambos os casos passaram):**
  - **Case 1 ($50 refeição):** rota `parse_expense → auto_approve → record_outcome`, `approved=True`, `approver=system`, zero chamadas de LLM, sem HITL. Exatamente o esperado.
  - **Case 2 ($150 jantar com cliente):** rota `parse_expense → security_screen (clean) → review_agent → human_approval`. O `review_agent` chamou `gemini-2.5-flash-lite`, avaliou risco como LOW com justificativa, e o pause de HITL foi disparado corretamente (evento `adk_request_input`, esperando aprovação humana). Exatamente o esperado.

**Atualização (2026-06-23) — Teste manual no Console Playground confirma HITL real, e Seção 9 (Observabilidade/Cloud Trace) concluída:**
- Testado manualmente no Playground do Console (sem o `--no-wait`/SDK) com o caso de $150: o Playground não tem um botão de aprovação pronto pra esquemas de função customizados, mas mostrou o evento bruto `adk_request_input` (function call) com `interruptId`, `response_schema` (`approved`, `approver`, `comment`) e a mensagem de alerta de risco — confirma visualmente, fora do script de smoke test, que o pause de HITL funciona de ponta a ponta. Resposta a esse interrupt (aprovar/rejeitar de fato) requer reenviar uma `functionResponse` casada com o `interruptId` via SDK — o Playground não expõe isso como formulário.
- **Cloud Trace (Seção 9) — análise de latência via API (Claude Code não tem acesso a navegador, então consultou a Trace API direto):**
  - **Case 1 ($50, auto-approve, trace `f6f5deb3...`):** 497ms total. Caminho 100% Python: `parse_expense` (99ms) → `auto_approve` (102ms) → `record_outcome` (92ms). Zero chamada de LLM. ~200ms de overhead de dispatch do Agent Runtime antes do workflow começar.
  - **Case 2 ($150, HITL, trace `7a9fa7c5...`):** 1767ms total — 3,6× mais lento que o caso 1. `parse_expense` (66ms) → `security_screen` (98ms, puro Python, scan de PII/injection) → `review_agent`/`call_llm` (`gemini-2.5-flash-lite`, **1129ms — 64% do tempo total**) → `human_approval` (96ms, só emite o interrupt e para).
  - **Lição pro Capstone:** o gargalo de latência num fluxo HITL nunca é o mecanismo de pausa em si (barato, ~100ms) — é sempre a chamada de LLM que acontece antes da decisão. Se performance for crítica, otimizar ali primeiro (modelo mais rápido / menos "thinking" na etapa de triagem).

**Atualização (2026-06-23) — BigQuery Analytics (Seção 9, item opcional): infraestrutura nunca foi aplicada, decisão de deixar pendente:**
- Confirmado via Python client (`bq` é WSL-only, então o Claude Code usou o client Python do `google-cloud-aiplatform` em vez do CLI): **nenhum dataset existe** em `kaggle-dia5-agent-runtime`. O `scaffold enhance` só gerou os arquivos **Terraform** (infraestrutura como código) — gerar o arquivo `.tf` não cria o recurso real; isso só acontece com `terraform apply`.
- **Nomes reais que seriam criados** (úteis pra quando for aplicado, no Capstone ou depois): dataset `ambient_expense_agent_telemetry`, tabela `gen_ai_client_inference_operation_details` (alvo do log sink), tabela externa `completions` (conector BigQuery Omni sobre o export de logs), view `completions_view` (a query SQL completa que vimos em `deployment/terraform/shared/completions.sql`).
- **Caminho pra aplicar, se decidir fazer depois:** `cd deployment/terraform/single-project && terraform init && terraform apply -var-file=vars/env.tfvars` (preenchendo `vars/env.tfvars` com projeto/região antes). Isso cria recursos novos cobráveis (dataset BigQuery, possivelmente sink de Cloud Logging) — não é grátis, mas é baixo custo pro volume de testes de um curso.
- **Decisão tomada:** deixar esse item pendente (não é obrigatório pra concluir o codelab nem pro badge) — registrado aqui pra retomar no Capstone se quiser métricas agregadas de aprovação/rejeição via BigQuery.
- **Lição da depuração:** `terraform plan`/arquivos `.tf` gerados ≠ infraestrutura existente — sempre checar se o `apply` realmente rodou antes de assumir que um recurso (dataset, tabela, bucket) já está provisionado.

**Atualização (2026-06-23) — Seção 10 (Agent Registry) confirmada, e decisão final sobre cleanup:**
- Verificado via consulta direta à API do Agent Registry (v1alpha) — já que `gcloud` é WSL-only — que o `ambient-expense-agent` foi **auto-registrado** no Agent Registry no primeiro deploy (2026-06-23 04:26 UTC) e atualizado no último redeploy (04:54 UTC). Protocolo `CUSTOM` (modo ADK, não A2A), framework `google-adk`, endpoints de query/stream apontando pro mesmo `reasoningEngine` do deploy.
- **Distinção de terminologia importante (fácil de confundir):** **Agent Registry** (inventário interno do GCP, auto-populado em todo `agents-cli deploy`, o que acabamos de confirmar) é diferente de **Gemini Enterprise** (produto separado, que exige um passo explícito de `agents-cli publish gemini-enterprise` + um "app" pré-criado lá). O agente **não está** no Gemini Enterprise — e não precisa estar, a menos que se queira expô-lo dentro do Gemini for Google Workspace. Vale lembrar essa distinção no Capstone se for preciso publicar pra esse público específico.
- **Decisão final sobre cleanup (Seção 11):** **não executado, de propósito.** Confirmado que o billing do Agent Engine cobra só por uso ativo de runtime (vCPU-hora/GB-hora), com cota grátis mensal — agente parado overnight não gera custo relevante de compute. Infraestrutura mantida ativa para a continuação no codelab de frontend (planejada para o dia seguinte, 2026-06-24).
- **Prompt de cleanup do codelab, guardado para quando for a hora certa (depois do codelab de frontend, ou se decidir não continuar):**
  ```text
  ...
  Artifact Registry.
  ```
  Esse prompt (rodado via Claude Code, ativando a skill `google-agents-cli-deploy`) faz 3 coisas: (1) lê o `deployment_metadata.json` pra achar o Agent Runtime ID e deleta a engine remota no Vertex AI; (2) remove o `deployment_metadata.json` local; (3) roda `gcloud artifacts docker images delete` pra limpar a imagem de container no Artifact Registry. **Lembrete:** não rodar antes de terminar de usar o endpoint — depois de deletado, precisa rodar `agents-cli deploy` de novo do zero.

**Status final do dia:** Codelab "Deploy ADK agent to Agent Runtime using Agents CLI" **100% concluído** — Seções 2 a 10 todas validadas (deploy, smoke test via SDK, teste manual no Playground, observabilidade via Cloud Trace, e confirmação de auto-registro no Agent Registry). Seção 11 (cleanup) conscientemente pulada. BigQuery Analytics (item opcional da Seção 9) fica pendente de aplicação do Terraform para uma sessão futura. Próxima sessão: codelab de frontend (Vibecode and Deploy a Frontend for an ADK Agent), reaproveitando o Agent Runtime já ativo.
...
- **Reasoning Engine / Agent Runtime ID:** identificador único do agente publicado no Vertex AI Agent Engine, no formato `projects/{n}/locations/{regiao}/reasoningEngines/{id}` — usado para referenciar o agente em produção (chamadas via SDK, console, etc.).
- **UNC path (`\\wsl$\...`):** convenção do Windows para acessar o filesystem do WSL como se fosse uma pasta de rede — permite que um processo Windows leia um arquivo que vive dentro do Linux, sem precisar copiá-lo.
